...
export class Editor extends React.Component {
    ...
    moveFormElement(dragElement, hoverElement) {
        ...
        const newFormElements1 = update(cards, {
            $splice: [
                [dragElement.index, 1],
    ...
    addIndexToCards(cards) {
        return cards.map((card, i) => {
            return Object.assign({}, card, { index: i });
        });
    }
    addCardsToList(lists, cards) {
        const IndexedCards = this.addIndexToCards(cards);
        return lists.map(list => {
            let cardsInList = [];
            for (let i = 0; i < IndexedCards.length; i++) {
                if (IndexedCards[i].listId === list.id)
                    cardsInList.push(IndexedCards[i]);
            }
            return {
                id: list.id,
                cards: cardsInList,
            ...
            };//Object.assign({}, list, { cards: cardsInList });
        })
